...
testthat::test_that("Test that the pressure correction produces expected results...", {
  ...
  press_cor <- mapply(function(k) calc_pressure_correction(k = k, temp_c = exp_val$input_conditions$TC, 
                                                           p_bar = exp_val$input_conditions$P), names(exp_val$check_values))
